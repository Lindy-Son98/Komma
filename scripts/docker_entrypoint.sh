...
function usage() {
    ...
    Usage: 
        ...
        $0 run -b binary_path -c config_path [Komma args]

    Options:
        build <PATH>:       Rebuilds KB from sources (provide absolute path to the source folder or repo.path file).
        run <args>:         Starts Komma. Arguments passed to this command will be redirected to Komma binary. You can set sc-server options from common config.
        ...
        EXTENSIONS_PATH can be set to specify the path for extensions used by Komma.
...
function start_machine {
    ...
    then
        ...
        "$BINARY_PATH"/Komma -c "$CONFIG_PATH" -e "$EXTENSIONS_PATH"
    else
        "$BINARY_PATH"/Komma "$@"
    fi
}
